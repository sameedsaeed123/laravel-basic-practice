...
export default function Sizes() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editId) {
                const res = await api.put(`/sizes/${editId}`, { name });
                showToast(res.data.message || 'Size updated successfully!', 'success');
            } else {
                const res = await api.post('/sizes', { name });
                showToast(res.data.message || 'Size created successfully!', 'success');
            }
            setName('');
            setEditId(null);
            fetchSizes();
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            showToast(res.data.message || 'Size deleted successfully!', 'success');
            fetchSizes();
        } catch (e) {
            showToast(e.response?.data?.message || 'Failed to delete size', 'error');
        }
    };
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold text-gray-800 mb-6">Sizes</h1>

            {showForm && (
                <div className="bg-white rounded-xl shadow-md p-6 mb-6">
                    <h2 className="text-lg font-semibold mb-4">{editId ? 'Edit Size' : 'Add Size'}</h2>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {sizes.length === 0 && (
                                <tr><td colSpan={(canEdit || canDelete) ? 3 : 2} className="px-6 py-8 text-center text-gray-400">No sizes found</td></tr>
                            )}
                            {sizes.map((size) => (
                                <tr key={size.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 text-sm text-gray-500">{size.id}</td>
                                    <td className="px-6 py-4 text-sm font-medium text-gray-800">{size.name}</td>
                                    {(canEdit || canDelete) && (
