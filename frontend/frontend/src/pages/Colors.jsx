...
export default function Colors() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editId) {
                const res = await api.put(`/colors/${editId}`, { name });
                showToast(res.data.message || 'Color updated successfully!', 'success');
            } else {
                const res = await api.post('/colors', { name });
                showToast(res.data.message || 'Color created successfully!', 'success');
            }
            setName('');
            setEditId(null);
            fetchColors();
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            showToast(res.data.message || 'Color deleted successfully!', 'success');
            fetchColors();
        } catch (e) {
            showToast(e.response?.data?.message || 'Failed to delete color', 'error');
        }
    };
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold text-gray-800 mb-6">Colors</h1>

            {showForm && (
                <div className="bg-white rounded-xl shadow-md p-6 mb-6">
                    <h2 className="text-lg font-semibold mb-4">{editId ? 'Edit Color' : 'Add Color'}</h2>
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="text-left px-6 py-3 text-sm font-semibold text-gray-600">Preview</th>
                                {(canEdit || canDelete) && <th className="text-right px-6 py-3 text-sm font-semibold text-gray-600">Actions</th>}
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                            {colors.length === 0 && (
                                <tr><td colSpan={(canEdit || canDelete) ? 4 : 3} className="px-6 py-8 text-center text-gray-400">No colors found</td></tr>
                            )}
                            {colors.map((color) => (
                                <tr key={color.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 text-sm text-gray-500">{color.id}</td>
                                    <td className="px-6 py-4 text-sm font-medium text-gray-800">{color.name}</td>
